  ...
  run: async (client, message, args, { GuildDB }) => {
    if (!message.member.voice.channel)
      return client.sendTime(
        message.channel,
        "❌ | **T'es pas la mais t'es ou ? Pas la !**"
      );
    if (
      ...
      return client.sendTime(
        message.channel,
        ":x: | **T'es pas la mais t'es ou ? Pas la !**"
      );
    ...
    if (!CheckNode || !CheckNode.connected) {
      return client.sendTime(
        message.channel,
        "❌ | **Eh.... Miro j'ai un probleme avec Lavalink**"
      );
    }
    ...
    try {
      if (SearchString.match(client.Lavasfy.spotifyPattern)) {
        ...
        if (!player)
          return client.sendTime(
            message.channel,
            "❌ | **Rien en lecture actuellment et j'ai rien dis :(**"
          );

        if (Searched.loadType === "NO_MATCHES")
          return client.sendTime(
            message.channel,
            "**J'ai rien trouver pour  **" + SearchString
          );
  ...
  SlashCommand: {
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!CheckNode || !CheckNode.connected) {
        return client.sendTime(
          interaction,
          "❌ | **Oups... Tu peux dire a Miro que j'ai un probleme avec Lavalink ?**"
        );
      }
